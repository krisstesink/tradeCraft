...
import idkhi.tradecraft.screen.GemPolishingScreenHandler;
...
public class GemPolishingStationBlockEntity extends BlockEntity implements ExtendedScreenHandlerFactory, ImplementedInventory {
    ...
    public GemPolishingStationBlockEntity(BlockPos pos, BlockState state) {
        super(idkhi.tradecraft.block.entity.ModBlockEntities.GEM_POLISHING_STATION_BLOCK_ENTITY, pos, state);
    ...
    public static void tick(World world, BlockPos pos, BlockState state, GemPolishingStationBlockEntity be) {
        if(world.isClient()) {
            return;
        }
